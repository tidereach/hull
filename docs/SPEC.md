...
│   ├── canary/                       (package: startup + scheduled self-test corpus; drift = freeze)
│   ├── integrity.py                  (pattern hash, model digest, prompt hash, dependency hash check)
│   ├── hook_manifest.py              (SHA-256 manifest of installed hooks; tamper detection at SessionStart)
...
│   ├── ner.py                        (opt-in spaCy NER pass; see §13.5.1)
│   ├── output_gate.py                (opt-in assistant-turn gating; see §13.5.2)
│   ├── sandbox.py                    (cross-layer preflight: assert neighbor sandbox is present and pinned)
