...
| 10 | Commit signing | **Deferred to v2 (amended 2026-06-30; see `ROADMAP.md` item 8).** The original v1 spec required `gitsign` (sigstore) on `main` across all five repos: keyless via OIDC (GitHub login) → ephemeral Fulcio cert → Rekor transparency log. The deferral is a pre-1.0 friction call — single-operator governance means signatures verify the operator to themselves; GitHub's squash-merge re-signs `main` with its own PGP key on landing; branch protection's `required_signatures` was already off because GitHub's native verifier doesn't accept Fulcio certs (the resolved ROADMAP item 6). The cost (browser-OIDC per push or `sign-push` alias maintenance) outweighs the v1 benefit. **Re-enable triggers (any one):** first non-operator contributor lands; the project commits to a SLSA target (L2 or higher) that requires signed commits; an enterprise consumer mandates signing; air-gapped dev becomes a requirement. The rationale for the original choice (eliminate per-developer key custody, tighten identity binding to OIDC, stay coherent with Tidereach's transparency-log-shaped audit chain) carries forward to the re-enable — when item 8 re-opens, gitsign is still the preferred direction, not GPG/SSH. Note: commit-signing is separate from interlock's Ed25519 (hook-manifest, audit-chain identity) — those remain operator-signed per Decision 1. Release-artifact signing (Decision 17 cosign) is **not** deferred and continues as specified. | 2026-06-29 user decision; **deferred 2026-06-30** |
...
- Author the canonical CI workflow templates in hull that every layer repo inherits — `.github/workflows/legacy-name-guard.yml` (Constraint 6), `.github/workflows/betterleaks.yml` (Decision 18(a); renamed from `gitleaks.yml` per the 2026-06-30 amendment), `.github/workflows/pr-title-lint.yml` (Decision 18(b)), ~~`.github/workflows/signature-verify.yml` (Decision 10 per-PR pass/fail signal)~~ (retired 2026-06-30 per ROADMAP item 8; Decision 10 deferred), `.github/workflows/image-sign.yml` (Decision 17 — airlock-only, reusable), `.github/workflows/ci-template.yml` (the example layer-repo CI file consumers copy at bootstrap), and `.github/workflows/release-template.yml` (the consumer-side `push: tags: ['v*']` caller for `image-sign.yml`; copied by image-publishing layers only). All third-party actions pinned to commit SHAs per Constraint 6's strict-pinning posture.
- Author the canonical pre-commit baseline `.pre-commit-config.yaml` covering betterleaks (Decision 18(a)), the standard hygiene set (trailing-whitespace, end-of-file-fixer, check-yaml, check-toml, check-added-large-files, …), `astral-sh/uv-pre-commit` for lockfile-freshness, and a commented-in-baseline `mirrors-mypy` block that each layer uncomments once its `src/` tree lands.
- Author `docs/REPO_SETTINGS.md` — the operator cookbook for the GitHub repo-side rules that complement the CI workflows: branch protection per Decisions 10 + 11 (~~require signed commits~~ (Decision 10 deferred 2026-06-30 per ROADMAP item 8), require status checks `legacy-name-guard / grep-gate`, `betterleaks / scan`, `pr-title-lint / lint`, ~~`signature-verify / verify`~~ (retired 2026-06-30), and the airlock-only `image-sign / build-sign-attest`, plus require linear history, require conversation resolution, restrict pushes per Decision 19); merge strategy = squash-only with PR title as commit title per Decisions 11 + 18(b); a `gh api`-based apply script; drift-detection commands.
- Author `docs/CI.md` — operator-facing overview of the CI infrastructure: the inheritance model (hull is the canonical home; layer repos consume via `uses: tidereach/hull/.github/workflows/<name>.yml@main`), the ~~six~~ five assertions (amended 2026-06-30; signature-verify retired per ROADMAP item 8) mapped to their workflow files, pre-commit baseline copy procedure, troubleshooting common failures (~~signature missing~~ (parked), betterleaks false positive, legacy-name hit needing exemption), maintenance cadence for SHA pin bumps.
...
*Gate:* `README.md` lists the four sibling layer repos + drydock; `migration/` planning docs preserved; **BLUEPAPER scaffold** (`docs/BLUEPAPER.md` or `docs/BLUEPAPER_TEMPLATE.md`) exists on `main`; `docs/GOVERNANCE.md` exists on `main`; **JETTISON scaffold** (`docs/JETTISON.md` or `docs/JETTISON_TEMPLATE.md`) exists on `main` (policy-module rule-authoring + baseline guide, per the 2026-06-29 layer-4 collapse); `docs/REPO_SETTINGS.md` exists on `main`; `docs/CI.md` exists on `main`; `.github/workflows/` contains the canonical workflows (`legacy-name-guard.yml`, `betterleaks.yml`, `pr-title-lint.yml`, `image-sign.yml`, `ci-template.yml`, `release-template.yml`); ~~the seven canonical workflows include `signature-verify.yml`~~ (retired 2026-06-30 per ROADMAP item 8; canonical workflow count drops from seven to six); `.pre-commit-config.yaml` exists on `main`; `CODEOWNERS` file is present with the wildcard rule (`* @dotknewt`) in each new repo's scaffold; canonical URL is `github.com/tidereach/hull`; legacy `dormant-warlock/spektralia` either 404s or redirects to the new URL.
